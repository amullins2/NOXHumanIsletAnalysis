//Please Note: This is a modified format of an ImageJ macro
//Originally developed by George Merces, Newcastle University, 04.11.2024
//Modified to save exocrine masks instead of subtracting endocrine regions
...
//NEW: Folder for endocrine binary masks
endocrineFolder = homeFolder + "Endocrine_Binary_Folder/";
if (File.isDirectory(endocrineFolder) < 1) {
	File.makeDirectory(endocrineFolder); 
}
//NEW: Folder for exocrine binary masks  
exocrineFolder = homeFolder + "Exocrine_Binary_Folder/";
if (File.isDirectory(exocrineFolder) < 1) {
	File.makeDirectory(exocrineFolder); 
}
//NEW: Folders for exocrine-masked channels
exocrineCh1Folder = homeFolder + "Exocrine_Ch1/";
if (File.isDirectory(exocrineCh1Folder) < 1) {
	File.makeDirectory(exocrineCh1Folder); 
}
exocrineCh2Folder = homeFolder + "Exocrine_Ch2/";
if (File.isDirectory(exocrineCh2Folder) < 1) {
	File.makeDirectory(exocrineCh2Folder); 
}
exocrineCh3Folder = homeFolder + "Exocrine_Ch3/";
if (File.isDirectory(exocrineCh3Folder) < 1) {
	File.makeDirectory(exocrineCh3Folder); 
}
exocrineCh4Folder = homeFolder + "Exocrine_Ch4/";
if (File.isDirectory(exocrineCh4Folder) < 1) {
	File.makeDirectory(exocrineCh4Folder); 
}
...
//MODIFIED SECTION: Create both endocrine and exocrine masks, then apply exocrine masks to all channels
...
for (i=0; i<l; i++) {
	...
	//Save ENDOCRINE mask (original thresholded image - white = endocrine)
	saveName = endocrineFolder + list[i];
	saveAs("Tiff", saveName);
	endocrineMask = getTitle();
	
	//Create and save EXOCRINE mask (inverted - white = exocrine)
	run("Duplicate...", "title=ExocrineMask");
	run("Invert");
	saveName = exocrineFolder + list[i];
	saveAs("Tiff", saveName);
	exocrineMask = getTitle();
	
	//Now apply exocrine mask to all channels
	//Channel 1
	fileName = Ch1 + list[i];
	open(fileName);
	run("8-bit");
	ch1Image = getTitle();
	imageCalculator("Multiply create", ch1Image, exocrineMask);
	saveName = exocrineCh1Folder + list[i];
	saveAs("Tiff", saveName);
	close();
	selectWindow(ch1Image);
	close();
	...
	run("8-bit");
	ch2Image = getTitle();
	imageCalculator("Multiply create", ch2Image, exocrineMask);
	saveName = exocrineCh2Folder + list[i];
	saveAs("Tiff", saveName);
	close();
	selectWindow(ch2Image);
	close();
	...
	run("8-bit");
	ch3Image = getTitle();
	imageCalculator("Multiply create", ch3Image, exocrineMask);
	saveName = exocrineCh3Folder + list[i];
	saveAs("Tiff", saveName);
	close();
	selectWindow(ch3Image);
	close();
	...
	run("8-bit");
	ch4Image = getTitle();
	imageCalculator("Multiply create", ch4Image, exocrineMask);
	saveName = exocrineCh4Folder + list[i];
	saveAs("Tiff", saveName);
	close();
	selectWindow(ch4Image);
	close();
	
	//Clean up
	selectWindow(endocrineMask);
	close();
	selectWindow(exocrineMask);
	close();
}

//Nuclear segmentation using StarDist on exocrine-masked nuclear channel
//Finds the names of image files within exocrine Ch3 folder
list = getFileList(exocrineCh3Folder);
l = list.length;
//Opens each image sequentially
for (i=0; i<l; i++) {
	roiManager("reset");
	//Opens the exocrine-masked nuclear image
	fileName = exocrineCh3Folder + list[i];
	open(fileName);
	getDimensions(width, height, channels, slices, frames);
	...
	if (n > 0) {
	    ...
	    for (j=0; j<n; j++) {
			...
			//Re-selects the ROI
			roiManager("select", j);
			//Shrinks the ROI down by 2 pixels
...
print("Endocrine masks saved to: " + endocrineFolder);
print("Exocrine masks saved to: " + exocrineFolder);
print("Exocrine-masked channels saved to respective folders:");
print("  - Channel 1: " + exocrineCh1Folder);
print("  - Channel 2: " + exocrineCh2Folder);
print("  - Channel 3: " + exocrineCh3Folder);
print("  - Channel 4: " + exocrineCh4Folder);
